...
`BriefReviewService` 使用 Jackson 3 的 `tools.jackson.databind.ObjectMapper` 和 `tools.jackson.core.type.TypeReference<List<String>>` 处理实体 JSON 字段。序列化或解析内部持久化数据失败时抛出：
...
- [x] 可以查询指定任务的全部 Brief 版本，结果按创建时间和版本 ID 倒序排列。
- [x] 可以查询指定任务的最新 Brief。
- [x] 人工修改始终创建新版本，不覆盖历史数据。
- [x] 新版本的父版本指向修改前的最新版本。
- [x] 历史版本不能被继续修改或批准。
- [x] 任务不在等待 Brief 审批状态时，修改和审批均返回 `TASK_STATUS_INVALID`。
- [x] Brief 与任务不匹配时返回 `INVALID_REQUEST`。
- [x] 首个占位 Brief 的创建人是 `SYSTEM`。
- [x] 人工版本记录 `createdBy`。
- [x] 批准版本记录 `approvedBy` 和 `approvedAt`。
- [x] 批准后任务状态为 `GENERATING`，Brief 状态为 `APPROVED`。
- [x] 批准后文案和图片状态仍为 `NOT_STARTED`。
- [x] 批准不会触发真实文案或图片生成。
- [x] 四个 HTTP 接口返回统一 `ApiResponse`。
- [x] DTO 不直接暴露 JPA 实体。
